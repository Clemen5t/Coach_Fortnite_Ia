...
+('True' if v4 else 'False'))
                if v6 is not None:args.append('-IPv6Enabled 
...
+('True' if v6 else 'False'))
            ...
            for row in value.get('interrupt') or []:
                ...
                if not keyword:continue
                if display:
                    out,err,code=_ps(f"Set-NetAdapterAdvancedProperty -Name '{name}' -RegistryKeyword '{keyword}' -DisplayValue '{display.replace(chr(39),chr(39)*2)}' -NoRestart -ErrorAction Stop")
                elif registry not in (None,'',[]):
                    vals=registry if isinstance(registry,list) else [registry]
                    vals=[str(x) for x in vals]
                    pv="@("+",".join("'"+x.replace("'","''")+"'" for x in vals)+")"
