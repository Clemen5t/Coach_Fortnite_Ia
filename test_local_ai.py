...
class Tests(unittest.TestCase):
    ...
    def test_local_image_request(self):
        ...
        self.assertEqual(path,'/api/generate');self.assertEqual(body['images'],['dGVzdA=='])
        self.assertFalse(body['stream']);self.assertNotIn('context',body)
    ...
    def test_staleness(self):
        self.assertTrue(usable('Couvert',.8,2,''))
        self.assertFalse(usable('Couvert',2.1,2,''))
        self.assertFalse(usable('SILENCE.',.8,2,''))
        self.assertFalse(usable('Couvert',.8,2,'Couvert'))
    def test_cancel_pending(self):
        ...
        self.assertTrue(Handler.entered.wait(2));self.client.cancel();thread.join(1)
    ...
    def test_generic_rejected(self):
        for text in ['Trouver une meilleure couverture immédiate.','Chercher une position de sécurité.','Chercher une couverture, vite, et stable.']:
            self.assertEqual(parse_advice(json.dumps({'category':'cover','evidence':'Le joueur est dehors','advice':text})),('SILENCE','none'))
    def test_category_cooldown(self):
        gate=AdviceGate();self.assertTrue(gate.allow('cover',100))
        self.assertFalse(gate.allow('cover',110))
        self.assertFalse(gate.allow('heal',105))
        self.assertTrue(gate.allow('heal',110))
        self.assertTrue(gate.allow('cover',131))
    def test_invalid_evidence(self):
        self.assertEqual(parse_advice('{"category":"cover","evidence":"","advice":"Construis un mur."}'),('SILENCE','none'))
