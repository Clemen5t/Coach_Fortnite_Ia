...
class Coach:
    def __init__(self,root):
        ...
        root.title('Coach Fortnite — Acolyte local — '+VERSION); root.geometry('900x900')
        ...
        ttk.Label(frame,text='Whisper + Gemma 3 Vision + Piper • local • aucune clé API').pack(anchor='w',pady=(0,12))

        voicebox=ttk.LabelFrame(frame,text='Acolyte vocal',padding=12); voicebox.pack(fill='x',pady=(0,12))
        ttk.Label(voicebox,text='F8 ou bouton : pose ta question, Acolyte regarde l’écran et répond.').pack(anchor='w')

        self.mic_choice=tk.StringVar(value=str(self.saved_settings.get('microphone','')))
        self.mic_devices={}
        self.mic_label=tk.StringVar(value='Micro : détection…')
        ...
        ttk.Label(voicebox,textvariable=self.voice_state).pack(anchor='w',pady=(8,3))

        meterrow=ttk.Frame(voicebox); meterrow.pack(fill='x')
        ttk.Label(meterrow,textvariable=self.mic_label,width=32).pack(side='left')
        self.meter=tk.Canvas(meterrow,height=34,width=450,highlightthickness=1,bg='#151515')
        ...
        for r,(label,var) in enumerate([('Écran',self.monitor),('Intervalle auto (s)',self.interval),('Rejeter après (s)',self.expiry),('Durée auto (min)',self.minutes)]):
            ttk.Label(grid,text=label).grid(row=r,column=0,sticky='w',pady=3); ttk.Entry(grid,textvariable=var,width=24).grid(row=r,column=1,sticky='w',padx=10)
        ttk.Checkbutton(autobox,text='Seulement quand Fortnite est au premier plan',variable=self.onlygame).pack(anchor='w')
        ttk.Checkbutton(autobox,text='Lire les conseils automatiques avec Piper',variable=self.speak).pack(anchor='w')
    ...
    def refresh_microphones(self,initial=False):
        ...
        try:
            ...
            self.mic_label.set('Micro sélectionné : '+selected.split(' — ',1)[-1][:45] if selected else 'Micro : aucun périphérique')
    ...
    def preview(self):
        try:
            ...
            im.thumbnail((960,540)); w=tk.Toplevel(self.root); w.title('Aperçu local')
            photo=ImageTk.PhotoImage(im); label=ttk.Label(w,image=photo); label.image=photo; label.pack()
    ...
    def configure_repo(self):
        ...
        if repo:
            try: updater.write_json(APP_DIR/'update-config.json',{'repo':updater.normalize_repo(repo),'branch':'main'})
    ...
    def update_app(self):
        ...
        self.updating=True; self.update_button.state(['disabled']); sid=self.session_id
        def work():
            try:
        ...
        try:
            while True:
                ...
                if kind=='audio_level': self.draw_audio(*data)
                    ...
                    self.voice_busy=False; self.talk_button.state(['!disabled']); self.voice_state.set('Prêt. F8 = parler au coach.')
                elif kind=='voice_error':
                    self.voice_busy=False; self.talk_button.state(['!disabled']); self.voice_state.set('Erreur : '+data[0])
                elif kind=='question': self.log.insert('end','\n🎙️ Toi : '+data[0]+'\n'); self.log.see('end')
                elif kind=='answer':
                    q,a,elapsed=data; self.advice.set(a); self.metric.set(f'Réponse vision : {elapsed*1000:.0f} ms'); self.log.insert('end','🤖 Acolyte : '+a+'\n'); self.log.see('end')
                elif kind=='status': self.status.set(data[0])
                elif kind=='finished': self.start_button.state(['!disabled'])
                elif kind=='result':
                    ...
                    if valid: self.advice.set(text)
                elif kind.startswith('update_'):
                    self.updating=False
                    if kind=='update_ok':
                        self.restart_required=True; messagebox.showinfo('Mise à jour','Version '+data[0]+' installée. Relance Coach Fortnite.')
                    else:
    ...
    def stop(self):
        self.stop_event.set(); self.status.set('Arrêt…')
